
export type College = {
   name: string;
   logo: string;
   color?: string;
}

export const colleges: College[] = [
   { name: 'Institute of Engineering & Technology(IET), Lucknow', 
      logo: 'https://drive.google.com/uc?export=view&id=1ykDncQTiC6P4F3_M-vb6JBRhNykFjwst' },

   { name: 'National Institute of Technology, Jaipur', 
      logo: 'https://drive.google.com/uc?export=view&id=1GUVzhH99lS_nprYBqsbx-t096aQaIwfS' },

   { name: 'National Institute of Technology, Calicut', 
      logo: 'https://drive.google.com/uc?export=view&id=1kdYTUaAPazCBPybh7wWWqxj8n42cKOwT' },

   { name: 'National Institute of Technology, Silchar', 
      logo: 'https://drive.google.com/uc?export=view&id=1b0J7STkWzl19xSxEH5JysXoUtI98I2xd' },
   
   { name: 'National Institute of Technology, Allahabad', 
      logo: 'https://drive.google.com/uc?export=view&id=1lT99PN8QxCiPTLF_o73lchulHURJTUuS' },

   { name: 'National Institute of Technology, Jalandhar', 
      logo: 'https://drive.google.com/uc?export=view&id=1UFXqfR8EDaUJSrNA8MHjdw2P5mR2K5Bv' },

   { name: 'National Institute of Technology, Kurukshetra', 
      logo: 'https://drive.google.com/uc?export=view&id=10y39LrfcOVPDrM95GFs0J1-BCmtGVRxP' },

   { name: 'National Institute of Technology, Delhi', 
      logo: 'https://drive.google.com/uc?export=view&id=1-43OYV0o8eaieGs7AVymQoFTM72wrkDU' },

   { name: 'National Institute of Technology, Hamirpur', 
      logo: 'https://drive.google.com/uc?export=view&id=1e4TT-UBFjySzdYX7ap8KICh5iAstlHos' },

   { name: 'National Institute of Technology, Agartala', 
      logo: 'https://drive.google.com/uc?export=view&id=1mF1mHYQtycZhhQQet3ozj9xxslh56sui' },

   { name: 'National Institute of Technology, Bhopal', 
      logo: 'https://drive.google.com/uc?export=view&id=1CqkkKYT8ExqMmBkcS7nt3eYqyFJPeMHe' },

   { name: 'National Institute of Technology, Patna', 
      logo: 'https://drive.google.com/uc?export=view&id=1ic4ZaYCxYiT8nMP7upKIXQQtm-ehtKFL' },

   { name: 'National Institute of Technology, Warangal', 
      logo: 'https://drive.google.com/uc?export=view&id=106PjYAr5f5m_ddDovbxXOQ3pLiVE7PGO' },

   { name: 'Sagar Institute of Research & Technology, Bhopal', 
      logo: 'https://drive.google.com/uc?export=view&id=1V5piy-OxsN0lqXUAA56joG_qJwhFFmvO' },

   { name: 'Vellore Institute of Technology', 
      logo: 'https://drive.google.com/uc?export=view&id=1qGGQwdxdrmjDRSZddhsCEXFk4Tqz7ifo' },
];